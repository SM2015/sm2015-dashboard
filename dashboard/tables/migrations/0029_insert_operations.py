...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        countries = [
            {'country': orm['core.Country'].objects.get(slug='belize'), 'name': 'BL-G1001', 'starting_date': date(2012,07,19), 'finish_date': date(2014,1,19) },
            {'country': orm['core.Country'].objects.get(slug='mexico') , 'name': 'ME-G1001', 'starting_date': date(2012,9,19), 'finish_date': date(2014,3,27) },
            {'country': orm['core.Country'].objects.get(slug='el-salvador') , 'name': 'ES-G1001', 'starting_date': date(2012,07,27), 'finish_date': date(2014,1,27) },
            {'country': orm['core.Country'].objects.get(slug='honduras') , 'name': 'HO-G1001', 'starting_date': date(2012,07,23), 'finish_date': date(2014,1,23) },
            {'country': orm['core.Country'].objects.get(slug='nicaragua') , 'name': 'NI-G1001', 'starting_date': date(2012,8,20), 'finish_date': date(2014,2,19) },
            {'country': orm['core.Country'].objects.get(slug='panama') , 'name': 'PN-G1001', 'starting_date': date(2012,12,19), 'finish_date': date(2014,6,19) },
            {'country': orm['core.Country'].objects.get(slug='guatemala') , 'name': 'GU-G1001', 'starting_date': date(2012,9,26), 'finish_date': date(2014,3,28) },
            {'country': orm['core.Country'].objects.get(slug='costa-rica') , 'name': 'CR-G1001', 'starting_date': date(2013,07,19), 'finish_date': date(2015,1,18) }
        ]
        for country in countries:
            orm.Operation.objects.create(
                    country = country.get('country'), 
                    name = country.get('name'),
                    starting_date = country.get('starting_date'),
                    finish_date = country.get('finish_date')
            )

    def backwards(self, orm):
        orm.Operation.objects.all().delete()
